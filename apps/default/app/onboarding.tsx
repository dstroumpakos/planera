...
export default function Onboarding() {
  ...
  const formatDateInput = (text: string, prev: string): string => {
    ...
    if (digits.length === 4) return isDeleting ? digits : `${digits}-`;
    if (digits.length === 5) return `${digits.slice(0, 4)}-${digits.slice(4)}`;
    if (digits.length === 6) return isDeleting ? `${digits.slice(0, 4)}-${digits.slice(4)}` : `${digits.slice(0, 4)}-${digits.slice(4)}-`;
